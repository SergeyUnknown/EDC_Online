...
function TreeView_GetNodeText(node) {
    ...
    if (trNode.childNodes[trNode.childNodes.length - 1].getElementsByTagName) {
        outernotes = trNode.childNodes[trNode.childNodes.length - 1].getElementsByTagName("A");
        if (!outernotes || outernotes.length == 0) {
            outernotes = trNode.childNodes[trNode.childNodes.length - 1].getElementsByTagName("SPAN");
        }
    }
    var textNode = (outernotes && outernotes.length > 0) ?
        outernotes[0].childNodes[0] :
...
function TreeView_ProcessNodeData(result, context) {
    ...
    if (result.length > 0) {
        ...
        if (__nonMSDOMBrowser) {
            ...
            newChildren = document.getElementById(treeNode.id + "notes");
        }
        else {
            table = WebForm_GetParentByTagName(treeNode, "TABLE");
            table.insertAdjacentHTML("afterEnd", chunk);
            newChildren = document.all[treeNode.id + "notes"];
        }
